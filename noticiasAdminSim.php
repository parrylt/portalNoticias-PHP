...
$validar = mysqli_query($con, "update materia set valida = 1 where id_materia = $id ;"); 

$cadastro = mysqli_query ($con, 'SELECT * FROM materia WHERE valida = 0');

    if (mysqli_num_rows($cadastro) === 0) 
    {
    echo "<h3 style='text-align: center; margin-top: 5%;'>Nenhuma notícia para aprovar!</h3>";
    } 
    else
    {
        echo '<h3 style="text-align: center; margin-top: 5%;">Você tem notícias para aprovar! <br><br>';
